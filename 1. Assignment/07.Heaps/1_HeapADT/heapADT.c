...
void reheapDown(HEAP* heap, int pos)
{
	...
	{
		int maxChild = 2 * pos + 1; //우선 left child값
		if (maxChild + 1 < heap->size && heap->data[maxChild] < heap->data[maxChild + 1]) //right child가 존재하고, leftchild보다 값이 크면 maxchild = rightchild
			maxChild++;
