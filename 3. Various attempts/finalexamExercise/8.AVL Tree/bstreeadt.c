...
int _bstHeight(TREE_NODE* root)
{
	...
	int l_val = _bstHeight(root->left);
	int r_val = _bstHeight(root->right);
	return 1 + ((l_val > r_val) ? l_val : r_val);
}
int bstNodes(BS_TREE* tree)
{
	if (tree)
		return _bstNodes(tree->root);
	else
		return 0;
}
int _bstNodes(TREE_NODE* root)
{
	int count = 0;
	if (root != NULL)
		count = 1 + _bstNodes(root->left) + _bstNodes(root->right);
	return count;
}
int bstLeafs(BS_TREE* tree)
{
	if (tree)
		return _bstLeafs(tree->root);
	else
		return 0;
}
int _bstLeafs(TREE_NODE* root)
{
	int count = 0;
	if (root != NULL)
	{
		if (root->left == NULL && root->right == NULL)
			...
			count = _bstLeafs(root->left) + _bstLeafs(root->right);
	}
	return count;
}
...
int getTreeWeight(TREE_NODE* node)
{
	...
	return 1 + getTreeWeight(node->left) + getTreeWeight(node->right);
}